...
def _eval_ssl_vpn_tls(rule, config):
    if config.search(r'config vpn ssl settings'):
        ...
        return rule._make_result(True, "SSL VPN TLS version: not restricted (default enabled)")
    return rule._make_result(True, "SSL VPN not configured; TLS restriction not required")
...
def _eval_log_encryption(rule, config):
    for block in config.get_blocks("log fortianalyzer setting"):
        algorithm = block.get("enc-algorithm", "").lower()
        reliable = block.get("reliable", "").lower()
        if algorithm == "high" and reliable == "enable":
            return rule._make_result(True, "FortiAnalyzer log encryption enabled")
        if not algorithm and reliable == "enable":
            return rule._make_result(True, "FortiAnalyzer enc-algorithm: high (default)")
        return rule._make_result(
            False,
            f"FortiAnalyzer enc-algorithm={algorithm or 'not set'}, "
            f"reliable={reliable or 'not set'}",
        )
    return rule._make_result(False, "FortiAnalyzer logging settings not configured")
