...
def _eval_intra_zone_deny(rule, config):
    zones = config.get_blocks("system zone")
    if not zones:
        return rule._make_result(True, "No system zones; default intra-zone behavior is deny")
    allowed_zones = [
        zone.name for block in zones for zone in block.sub_blocks
        if zone.get("intrazone", "").lower() == "allow"
    ]
    if allowed_zones:
        return rule._make_result(False, f"intrazone allow configured: {', '.join(allowed_zones)}")
    return rule._make_result(True, "No intrazone allow found; default is deny")
...
def _eval_usb_disable(rule, config):
    auto_install = config.get_block("system auto-install")
    if auto_install:
        config_install = auto_install.get("auto-install-config", "").lower()
        image_install = auto_install.get("auto-install-image", "").lower()
        if config_install == "disable" and image_install == "disable":
            return rule._make_result(True, "USB config and image auto-install disabled")
        return rule._make_result(
            False,
            f"auto-install-config={config_install or 'not set'}, "
            f"auto-install-image={image_install or 'not set'}",
        )
    return rule._make_result(
        True,
        "config system auto-install not found; default is disable for config and image auto-install",
    )


def _eval_static_keys_tls(rule, config):
    val = config.get_global_setting("ssl-static-key-ciphers", "")
    if val.lower() == "disable":
        return rule._make_result(True, "Static TLS key ciphers disabled")
    return rule._make_result(
        False,
        f"ssl-static-key-ciphers: {val or 'enable (default)'}",
    )
...
def _eval_global_strong_encryption(rule, config):
    ...
    if not val:
        return rule._make_result(True, "strong-crypto: enable (default)")
    if val.lower() == "enable":
...
def _eval_tls_management_gui(rule, config):
    ...
    if not val:
        return rule._make_result(
            True,
            "admin-https-ssl-versions: tlsv1-2 tlsv1-3 (default)",
        )
    if val.lower().split() == ["tlsv1-3"]:
        return rule._make_result(True, "Management GUI restricted to TLS 1.3")
...
def _eval_cdn(rule, config):
    ...
    if not val:
        return rule._make_result(True, "gui-cdn-usage: enable (default)")
    if val.lower() == "enable":
        return rule._make_result(True, "CDN enabled for GUI")
    return rule._make_result(False, f"gui-cdn-usage: {val or 'not set'}")


def _eval_single_cpu_logging(rule, config):
    val = config.get_global_setting("log-single-cpu-high", "")
    if not val:
        return rule._make_result(True, "log-single-cpu-high: enable (default)")
    if val.lower() == "enable":
        return rule._make_result(True, "Single CPU overload logging enabled")
    return rule._make_result(False, f"log-single-cpu-high: {val or 'not set'}")


def _eval_hostname_hidden(rule, config):
    val = config.get_global_setting("gui-display-hostname", "")
    if not val:
        return rule._make_result(True, "gui-display-hostname: disable (default)")
    if val.lower() == "disable":
        return rule._make_result(True, "Hostname hidden on login GUI")
    return rule._make_result(False, f"gui-display-hostname: {val or 'not set'}")
...
def _eval_admin_lockout(rule, config):
    ...
    if not threshold and not duration:
        return rule._make_result(True, "Lockout: threshold and duration enabled by default")
    try:
        valid = int(threshold) <= 3 and int(duration) <= 900
...
def _eval_encrypted_access(rule, config):
    issues = []
    for block in config.get_blocks("system interface"):
        for interface in block.sub_blocks:
            access = interface.get("allowaccess", "").lower().split()
            insecure = [service for service in ("http", "telnet") if service in access]
            if insecure:
                issues.append(f"{interface.name}: {', '.join(insecure)}")
    if not issues:
        return rule._make_result(True, "HTTP and Telnet are not allowed on interfaces")
    return rule._make_result(False, f"Unencrypted access enabled: {'; '.join(issues)}")
...
def _eval_av_updates(rule, config):
    if config.search(r'config antivirus'):
        ...
        return rule._make_result(True, "Antivirus configured but updates not scheduled (default enabled)")
...
def _eval_event_logging(rule, config):
    block = config.get_block("log eventfilter")
    if block and block.get("event", "").lower() == "enable":
        return rule._make_result(True, "All event logging enabled")
    if not block:
        return rule._make_result(True, "config log eventfilter set event enable not found (default enabled)")
    return rule._make_result(False, "config log eventfilter event is not enabled")


def _eval_fortianalyzer_logging(rule, config):
    blocks = config.get_blocks("log fortianalyzer setting")
    syslog_blocks = config.get_blocks("log syslog setting") + config.get_blocks("log syslogd setting")
    if any(block.get("status", "").lower() == "enable" for block in blocks + syslog_blocks):
        return rule._make_result(True, "Centralized logging destination configured")
    return rule._make_result(False, "Centralized logging destination not configured")


def _eval_syslog(rule, config):
    blocks = config.get_blocks("log syslog setting") + config.get_blocks("log syslogd setting")
    if any(block.get("status", "").lower() == "enable" for block in blocks):
        return rule._make_result(True, "Syslog logging enabled")
    return rule._make_result(False, "Syslog logging not configured")
...
def get_level1_rules() -> List[CISRule]:
    ...
    rules = [
        ...
        CallableCISRule(
            ...
            description="Intra-zone traffic should not be universally allowed.",
            expected_value="intrazone: deny",
            remediation="Configure intra-zone handling under config system zone",
            category="Network", cis_section="1.2",
            remediation_cli="config system zone\n  edit <zone>\n    set intrazone deny\n  next\nend",
        ),
        CallableCISRule(
        ...
        CallableCISRule(
            ...
            expected_value="auto-install-config: disable and auto-install-image: disable",
            remediation="Configure USB auto-install under config system auto-install",
        ...
        CallableCISRule(
            ...
            remediation="Configure admin-https-ssl-versions to tlsv1-3",
        ...
        CallableCISRule(
            _eval_single_cpu_logging,
            rule_id="2.1.12",
            title="Ensure single CPU core overloaded event is logged",
            level=L1, severity=RuleSeverity.MEDIUM,
            description="Log events when a single CPU core is overloaded.",
            expected_value="log-single-cpu-high: enable",
            remediation="Enable single CPU core overload logging",
            category="System", cis_section="2.1.12",
            remediation_cli="config system global\n  set log-single-cpu-high enable\nend",
        ),
        CallableCISRule(
            _eval_hostname_hidden,
            rule_id="2.1.13",
            title="Ensure Hostname is Not Displayed On Login GUI",
            level=L1, severity=RuleSeverity.LOW,
            description="Do not expose the device hostname on the login GUI.",
            expected_value="gui-display-hostname: disable",
            remediation="Disable hostname display on the login GUI",
            category="System", cis_section="2.1.13",
        ...
        CallableCISRule(
            ...
            category="Logging", cis_section="7.3.2",
