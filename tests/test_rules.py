...
class TestLevel1Rules:
    ...
    def test_fortios_74_intra_zone_syntax(self):
        config = FortiGateConfigParser().parse_content("""
config system zone
    edit "SERVIDORES"
        set intrazone deny
    next
...
""")
        rule = next(r for r in get_level1_rules() if r.rule_id == "1.2")
        assert rule.evaluate(config).passed

    def test_intra_zone_defaults_to_deny_when_unset(self):
        config = FortiGateConfigParser().parse_content("""
config system zone
    edit "SERVIDORES"
        set interface "port1"
...
""")
    ...
    def test_fortios_74_auto_install_syntax(self):
        config = FortiGateConfigParser().parse_content("""
...
""")
    ...
    def test_auto_install_defaults_to_disabled_when_unset(self):
        config = FortiGateConfigParser().parse_content("config system global\nend\n")
        rule = next(r for r in get_level1_rules() if r.rule_id == "2.1.7")
        assert rule.evaluate(config).passed
    ...
    def test_fortios_74_defaults_for_system_settings(self):
        config = FortiGateConfigParser().parse_content("config system global\nend\n")
        expected_passes = {"2.1.10", "2.1.11", "2.1.12", "2.1.13"}
        for rule in get_level1_rules():
            if rule.rule_id in expected_passes:
                assert rule.evaluate(config).passed

    def test_static_tls_keys_default_is_not_compliant(self):
        config = FortiGateConfigParser().parse_content("config system global\nend\n")
        rule = next(r for r in get_level1_rules() if r.rule_id == "2.1.8")
        assert not rule.evaluate(config).passed
...
class TestLevel2Rules:
    ...
    def test_log_encryption(self, config):
        rule = next(r for r in get_level2_rules() if r.rule_id == "7.3.1")
        result = rule.evaluate(config)
        assert result.passed
